...
def main_worker(gpu, args):
    ...
    if args.cocoop:
        ...
        model = get_coop(args.arch, args.test_sets,
                         args.gpu, args.n_ctx, args.ctx_init)
...
def test_time_adapt_eval(val_loader, model, model_state, optimizer, optim_state, scaler, args):
    ...
    for i, (images, target) in enumerate(val_loader):
        ...
        if not args.cocoop:  # no need to reset cocoop because it's fixed
            ...
            optimizer = None
            pgen_ctx = test_time_tuning(
                model, (image_feature, pgen_ctx), optimizer, scaler, args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resolution', default=224,
                        type=int, help='CLIP image resolution')
    ...
    parser.add_argument('--tpt', action='store_true',
                        default=False, help='run test-time prompt tuning')
    parser.add_argument('--selection_p', default=0.1,
                        type=float, help='confidence selection percentile')
    parser.add_argument('--tta_steps', default=1, type=int,
                        help='test-time-adapt steps')
    parser.add_argument('--n_ctx', default=4, type=int,
                        help='number of tunable tokens')
    parser.add_argument('--ctx_init', default=None,
                        type=str, help='init tunable prompts')
    parser.add_argument('--cocoop', action='store_true', default=False,
                        help="use cocoop's output as prompt initialization")
    parser.add_argument('--load', default=None, type=str,
                        help='path to a pre-trained coop/cocoop')
