...
from src.models.common import select_confident_samples
...
class TPTPromptLearner(nn.Module):
    def __init__(
        ...
        tokenizer = open_clip.get_tokenizer(arch)  # type: ignore
...
class TPTModel(nn.Module):
    def __init__(
        ...
        self.visual: open_clip.transformer.VisionTransformer = clip_model.visual  # type: ignore
        self.visual.eval()
    ...
    def _forward_image(self, x: torch.Tensor) -> torch.Tensor:
        ...
        pooled, tokens, x = self._pool(x)  # type: ignore
    ...
    def __encode_image(
        ...
        return F.normalize(pooled_pre_norm, dim=-1) if normalize else pooled_pre_norm  # type: ignore
    ...
    def forward(self, image: torch.Tensor) -> torch.Tensor:
        ...
        return logits, image_features  # type: ignore
...
class TPT(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        for step in range(self.tpt_steps):
            with torch.cuda.amp.autocast():
                ...
                loss = self._avg_entropy_loss(logits)
    ...
    def _avg_entropy_loss(self, outputs: torch.Tensor) -> torch.Tensor:
        """
...
class TPTTop10(TPT):
    def __init__(
        ...
        super().__init__(pretrained, arch, class_names, tta_steps, lr, device)
